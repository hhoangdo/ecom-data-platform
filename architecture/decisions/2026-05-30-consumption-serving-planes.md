...
| Reconciled analytical serving | Executive teams and historical BI | MinIO Bronze -> Spark Silver/Gold tables | Trino canonical SQL over Gold tables, plus a DuckDB local KPI mart generated from Gold | within 1 hour | Reconciled business truth for KPI reporting |
...
- DuckDB is included for coursework portability: it gives a local, file-based OLAP mart that can be regenerated from Gold tables without requiring a shared warehouse service.
...
| DuckDB executive mart | Easy local demo and coursework inspection path | Not the canonical multi-user executive warehouse |
...
- Define the hourly DuckDB mart generation flow from Gold tables, including which executive KPI tables are exported.
